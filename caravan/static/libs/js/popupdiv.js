...
/**
 * Function Object for showing html content in a non-modal div (i.e., which doesn't "freeze" all user inputs until they it is showing) thus
 * emulating common popup-dialogs for showing messages or processing user input. 
 * <p>Requires jQuery. Works in IE7+, FF, Chrome (<i>old note:<s>In IE7 some dimensions
 * do not span the whole optimized width. Probably due to a bug in calculating the size when scrollbars are present. It works however</s></i>).<p>
 * This class builds an absolutely positioned div for popup forms, message dialogs or listitem popup (emulating the
 * &lt; select &gt; tag element popup). If you're scared about the doc, scroll below to see some examples.
 * <p>Usage:
 * <pre>var p = new PopupDiv(dictionary); p.show();</pre> or simply <pre>new PopupDiv(dictionary).show();</pre>
 * dictionary is an object with the following property names (in brackets the default associated value if missing). None of them is mandatory, but
 * at least the property 'content' should be specified (unless showing an empty div is what you want to get)
 * <ul>
 * <li>content (""): the popup content. Can be:
 *      <ul><li> a dictionnary of key:val pairs (form filling popup). Each pair represents a row in the popup. The row will be a div with
 *          a &lt; span &gt; with innerHTML=key followed by an &lt; input &gt; with value = val (&lt; span &gt;s and &lt; input &gt;s 
 *          horizontal alignement is automatically calculated so there should be
 *          no need of extra css). The type of &lt; input &gt; is determined as follows:
 *          <ul><li> val is boolean: &lt; input type=checkbox &gt;
 *          <li> val is an array of strings: &lt; select &gt; tag (non multi select. Yes, it is not an &lt; input &gt; tag in the strict term...)
 *          <li> otherwise: &lt; input type=text &gt; with val.toString as value.
 *          <li>PENDING: Add Number, Date and so-on!!!
 *          </ul></li>
 *          If the parameter showOk is true (see below), a click on the ok button will trigger the popup onOk callback (see below) with
 *          argument a dictionary of (key: &lt; input &gt; value) pairs
 *      <li> an array of strings for (list item popup). Each array element S (string) will be represented by a row of the popup (internally, 
 *      an anchor with innerHTML=S)</li> 
 *      A click on each anchor triggers the onOk callback (see onOk below), with argument an object of the form {selIndex:N},
 where N is the index of the anchor being clicked
 *      <li> a jQuery object: the content will be appended to the popup
 *      <li> otherwise: content.toString() will be set as the popup innerHTML</li>
 *      <p>
 *      In all of these cases, object inserted in the popup via the content property can be retrieved and manipulated via the popup.find method 
 *      (same syntax as in jQuery)
 * </ul>
 * <li>invoker (jQuery(window)): a jQuery object representing an html element. If invoker is:
 *        <ul><li>a clickable element (anchor, button, input of type button or submit), then the PopupDiv will behave as a popup dialog 
 *        <!--&lt; select &gt; popuplist--> of invoker.
 *        Thus, when showing, the PopupDiv will calculate the available space nearby invoker to show up in the window corner next to invoker which best fits 
 *        the PopupDiv size. In this case the parameter focusable (see below) is usually set to true
 *        <li>Otherwise, the popupdiv will be centered inside invoker. Note that <!--internally--> each popupdiv is appended to the body element, so it will
 *            be visually centered in invoker but it does not belong to invoker children hierarchy 
 *            (unless invoker is the jQuery('body') element). In this case the parameters bounds and/or boundsExact (see below) might be also specified
 * <li>bounds ({'top':0.25, 'left':0.25, 'right':0.25, 'bottom':0.25}): if invoker is a clickable element, it is ignored. Otherwise, specifies the
 ...
 *      size: top, left, right, bottom). Each bounds element can be in percentage of invoker size if lower than 1 
 *      (eg, bounds.left = 0.25: popup left margin is 25% of invoker width, and so on) or as pixel measure (if greater than 1)
 * <li>boundsExact (false): if invoker is a clickable element, it is ignored. Otherwise, specifies whether bounds (see above) should be a hint
 ...
 *      determined by <!--invoker size and--> the property bounds. Otherwise, the maximum popup size will be R, and if the popup size is included in R, it
 *      will be centered in R. NOTE: css padding, margin and border, if set via the property popupClass or popupCss (see below) might alter
 *      the real height and width of the rectangle <!--(those properties are ADDED to the natural height/width of the popup)-->
 * <li>focusable (false): boolean. When true, the popup gains the focus when shown, and disappears when looses the focus
 *      (popup.close() is called, see below)
 * <li>showOk (false): determinesy whether or not an ok button should be shown at the bottom of the popup. The ok button is an &lt; a &gt; tag whose
 *      click will trigger the popup.onOk callback (see below). This parameter should always be true for form filling popup (see PopupDiv.content above) 
 *      and when onOk is specified (see below)
 * <li>okButtonTitle ('Ok') [see note4]: self-explanatory
 * <li>onOk (null): callback. callback to be executed when the ok button is pressed. When specified, showOk must be set to true.
 ...
 *        <pre>popup.setContent("&lt; input type='text' "+popup.getFormDataAttrName()+"='default value'/ &gt;");popup.showOk=true;</pre>
 *        After each onOk callback has been executed, popup.close() will be always called
 * <li>onShow (null): callback to be executed when the popup shows up
 * <li>defaultCloseOperation ('hide'): specified what to do when popup.close() is called. 'remove' calls jQuery.remove() on the popup, ie it removes the html element from the document, 'hide' or any other value simply call jQuery.hide() on the popup
 * <li>onClose (null): callback to be executed when the popup .close() function is called. The callback must take one argument (string) which
 ...
 * <li>showClose (false): a parameter specifying whether a close button should appear on the top-right corner of the popup. Clicking the close button
 *      (internally, an &lt; a &gt; tag) will close the popup and trigger popup.close() (and associated callbacks bindings, if any)
 * <li>closeButtonTitle ('x') [see note4]: self-explanatory
 * <li>title (""): a parameter specifying whether the popup should have a title. The title will be placed on the top of the popup.
 * <li>shadowOffset (4): the shadow offset, in pixels. Each popup has a 'shadow' which renders a kind of 3d raised effect. Set to 0 if no shadow must be visualized
 * <li>p.okButtonAlign ('right'): self-explanatory. Takes the same argument as css text-align property. The css property text-align is set on the ok button parent div, so if okButtonClass (see below) is specified it might override the button alignement behaviour
 * <li>popupClass ("") [see note1+2]: the popup class(es). The top and bottom divs (housing title/close and ok buttons respectively) are not affected by this parameter
 * <li>popupCss ({}) [see note1+3]: the popup css. The top and bottom divs (housing title/close and ok buttons respectively) are not affected by this parameter
 * <li>okButtonClass ('') [see note1+2+4]: the ok button class
 * <li>closeButtonClass ('') [see note1+2+4]: the close button class
 * <li>titleClass ('') [see note1+2]: the title class
 * <li>listItemClass ('') [see note1+2]: the list items css, valid only if the popup is a listitem popup (see content above):
 *         it applies to each popup row (internally, an &lt; a &gt; tag with display block)
 * <li>listItemCss ('') [see note1+3]: the list items css, valid only if the popup is a listitem popup (see content above):
 *        it applies to each popup row (internally, an &lt; a &gt; tag with display block)
 * <li>fadeInTime ('fast'): the fade in time when popup.show() is called. See jQuery show for possible values (briefly, a number in milliseconds or the string 'fast' or 'slow')
 * <li>fadeOutTime (0): the fade out time when popup.close() is called. See jQuery show for possible values (briefly, a number in milliseconds or the string 'fast' or 'slow')
 * <li>shadowOpacity (0.25): self-explanatory. 1 means shadow completely black, 0 completely transparent (bascially, no shadow)
 * <li>zIndex (10000): the popup zIndex. Should be left untouched unless there are issues with other component with hight zIndex.
 * </ul>
 * <p>
 * <br>[note1] IMPORTANT: For every css or class parameter, some css styles might be overridden before showing the popup because they would interfere with the correct placement and
 ...
 * <br>[note2]: class arguments are in the same form of jQuery.addClass() argument (ie, a string which can denote also multiple classes separated by spaces)
 * <br>[note3]: css arguments are in the same form of jQuery.css() argument (ie, an object of cssName:cssValue pairs)
 * <br>[note4]: to customize ok button (closeButton respectively) with css (eg, with an icon), specify a background image in the class AND an height and a width (or padding),
