...
class Gmpe(object):
    ...
    def __init__(self, **kwargs):
        ...
        if not hasattr(self, 'm'):
            raise Exception('{0} error: missing magnitude \'m\''.format(self.__repr__()))
        
        if self.m > self.m_bounds[1] or self.m < self.m_bounds[0]:
            raise Exception("{0} error: magnitude 'm' ({1:f}) not in {2}".format(self.__repr__(), self.m, str(list(self.m_bounds) ) )) #because list str is like closed interval in math
